...
import PencilFillSvg from '@/shared/icons/Pencil.Fill'
...
const ModalMyProfile = ({ show, toggleShow }: ModalMyProfileProps) => {
    const [showProfilePictureMenu, setShowProfilePictureMenu] = useState<boolean>(true)
    const profilePictureRef: any = useRef(null)
    const profilePictureInputRef: any = useRef(null)
    const profilePictureMenuRef: any = useOnClickOutside(() => setShowProfilePictureMenu(false))

    const form = useFormik({
        initialValues: {
            description: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Quibusdam itaque incidunt labore quasi quidem praesentium illo quisquam maxime alias! Harum illum voluptatum molestiae numquam quas, explicabo reiciendis cumque veniam, cupiditate ad beatae perspiciatis. Natus doloremque dolorem a provident quibusdam. Hic vel nostrum nemo beatae aliquam excepturi quod soluta dolore iure.',
    ...
    const onChangeProfilePictureInput = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        if (
            ...
            profilePictureRef.current.setAttribute('src', URL.createObjectURL(event.target.files[0]))
    ...
    return (
        ...
        >
            <form
                className='p-5 px-8 pt-0 flex flex-col gap-y-6'
                ...
                <div className='flex flex-col items-center gap-y-1'>
                    <div className='relative cursor-pointer group'>
                        ...
                        <input
                            ...
                            name='profilePicture'
                            accept='image'
                            onChange={(event) => onChangeProfilePictureInput(event)}
                        ...
                        <div
                            className={`absolute bg-white rounded-lg border left-0 top-[50%] border-black/10 ${showProfilePictureMenu ? 'visible' : 'invisible'}`}
                        ...
                        >
                            ...
                            <button className='w-full px-5 py-3 text-center bg-white text-slate-800 whitespace-nowrap hover:brightness-95'>Delete</button>
                        </div>
                    </div>
