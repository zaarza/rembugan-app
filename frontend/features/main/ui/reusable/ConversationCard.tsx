import DoubleCheckMarkSvg from "@/shared/icons/DoubleCheckMark";
...
const ConversationCard = ({ name, content, avatar, position, time, is_readed }: ConversationCardProps) => {
    return (
        <div className={`flex flex-col ${position === "RIGHT" && "items-end"}`}>
            <div className="grid grid-cols-[auto_auto_auto] grid-rows-2 w-fit gap-x-4 gap-y-1">
                ...
                {is_readed !== undefined && (
                    <div className={`place-self-end w-5 row-start-2 ${position === "LEFT" ? "col-start-3" : "col-start-1"} ${is_readed ? "first:fill-primary" : "first:fill-slate-500"}`}>
                        <DoubleCheckMarkSvg />
                    </div>
                )}
                ...
                <img className={`w-11 aspect-square rounded-lg row-start-2 border border-black/10 ${position === "LEFT" ? "col-start-1" : "col-start-3"}`} src={avatar} alt={`${name} profile picture`} />
            </div>
        </div>
    );
};
