...
const ConversationCard = ({ name = '', message, avatar, position, time, is_seen }: ConversationCardProps) => {
    return (
        <div className={`flex flex-col max-w-[400px] ${position === 'RIGHT' && 'self-end'}`}>
            <div className='grid grid-cols-[auto_auto_auto] grid-rows-1 w-fit gap-x-4 gap-y-1'>
                ...
                {is_seen != undefined && (
                    <div
                        className={`place-self-end w-5 row-start-2 col-start-2 ${
                            is_seen === 1 ? 'first:fill-primary' : 'first:fill-slate-500'
                        }`}
                    >
