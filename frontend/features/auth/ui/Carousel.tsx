...
const Carousel = () => {
    ...
    return (
        <div className="carousel bg-primary flex flex-col items-center px-8 py-12 gap-y-10 lg:w-full lg:order-2 justify-center">
            <img className="h-[200px]  rounded-lg shadow-lg lg:h-[350px]" src={data[active].image} alt="carousel" />
            ...
            <div className="flex gap-x-3">
                {data.map((carouselItem, index) => (
                    <button className={` rounded-full w-3 aspect-square ${active === index ? "bg-white" : "bg-white/50"}`} key={index} onClick={() => setActive(index)} />
                ))}
            </div>
        </div>
    );
