...
type ConversationHeaderProps = {
    ...
    setShowChatProfileDetail: () => void;
}

const ConversationHeader = ({ name, status, profilePicturePath, setShowChatProfileDetail }: ConversationHeaderProps) => {
    const {setShowConversation} = useAppStore((state: any) => ({setShowConversation: state.setShowConversation}));
    ...
    return (
        <div className="sticky top-0 flex items-center justify-between px-3 py-3 bg-white border border-l-0 title lg:px-8 border-b-black/10">
            ...
            <button className="w-8 group " onClick={() => setShowChatProfileDetail()}>
                <svg className="fill-slate-800 group-hover:fill-primary" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
